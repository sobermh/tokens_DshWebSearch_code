...
import { FETCH_TIMEOUT_MS, PER_ENGINE_MS, hedgedRace } from "./hedged-race.js";
...
// 单次抓取：外部取消与超时通过 AbortSignal.any 合并。
// 不再重试——对同一个卡住的地址重试只是把损失翻倍，冗余由 search() 的引擎竞速提供。
async function fetchHtml(url, signal, timeoutMs = FETCH_TIMEOUT_MS) {
  let response;
  try {
    // AbortSignal.any 对"已经 abort 的源"会立即生效；早先的 addEventListener 写法不会，
    // 于是取消之后发出的重试会拿到一个全新的、不受上游预算约束的计时器。
  ...
  // 截断/空壳页按失败处理（原先由重试包装器判定，这里内聚进来保持调用方语义不变）
  if (html.length <= 500) {
    throw new Error(`empty response (${html.length} bytes)`);
  }
  return html;
}
...
function apply(ctx, config) {
  ...
  const provider = {
    ...
    async search(request, signal) {
      ...
      // 每个引擎的真实失败原因。旧实现只抛一句统一文案，真正卡住的是谁完全查不出来。
      const failures = [];

      // 引擎竞速（hedged request）。旧的串行回退有个致命点：一个引擎挂住就吃光整条链的
      // 总预算，排在后面的引擎一个请求都发不出去——冗余等于零。竞速把总延迟从"所有失败者
      // 之和"变成"最快那个健康引擎"，挂住的引擎只是一个永远不会赢的 promise，代价归零。
      const leader = chain[0];
      // 只让免费引擎参与竞速：付费/带 key 的引擎并发打会产生费用，留作兜底串行尝试。
      const racers = chain.filter((e) => e !== leader && freeEngines.includes(e));
      const lastResort = chain.filter((e) => e !== leader && !freeEngines.includes(e));

      const callEngine = async (engine, engineSignal) => {
        if (engine === "ddg") {
          return await searchDdgHtml(request.query, request.maxResults, { ...cfg, timeRange }, engineSignal);
        }
        if (engine === "ddg-lite") {
          return await searchDdgLite(request.query, request.maxResults, { ...cfg, timeRange }, engineSignal);
        }
        if (engine === "bing") {
          return await searchBing(request.query, request.maxResults, cfg, engineSignal);
        }
        if (engine === "searxng") {
          return await searchSearxng(request.query, request.maxResults, { ...cfg, timeRange }, engineSignal);
        }
        if (engine === "anysearch") {
          return await searchAnysearch(request.query, request.maxResults, engineSignal);
        }
        if (engine === "exa") {
          // exa：有 key 走 REST，无 key 走 keyless MCP（免费）
          const key = await resolveApiKey("EXA_API_KEY", "exaApiKey");
          return key
            ? await searchExa(request.query, request.maxResults, key, timeRange, engineSignal)
            : await searchExaMCP(request.query, request.maxResults, engineSignal);
        }
        if (engine === "tavily") {
          // tavily：有 key 走账号档，无 key 走 keyless（免费匿名额度）
          const key = await resolveApiKey("TAVILY_API_KEY", "tavilyApiKey");
          return await searchTavily(request.query, request.maxResults, key, timeRange, engineSignal);
        }
        if (engine === "keenable") {
          // keenable：有 key 走 REST，无 key 走 keyless MCP（免费）
          const key = await resolveApiKey("KEENABLE_API_KEY", "keenableApiKey");
          return await searchKeenable(request.query, request.maxResults, key, timeRange, engineSignal);
        }
        if (engine === "perplexity") {
          ...
          return await searchPerplexity(request.query, request.maxResults, key, engineSignal);
        }
        if (engine === "deepseek-official") {
          const key = await resolveApiKey("DEEPSEEK_API_KEY", "deepseekApiKey");
          if (!key) throw new Error("DEEPSEEK_API_KEY is not configured");
          return await searchDeepSeekOfficial(request.query, request.maxResults, key, engineSignal);
        }
        throw new Error(`unknown engine "${engine}"`);
      };

      // 单个引擎跑一次：外部取消、竞速胜出（raceSignal）、单引擎上限，任一触发都立即中止。
      const runEngine = async (engine, raceSignal) => {
        const engineSignal = AbortSignal.any([
          ...(signal !== undefined ? [signal] : []),
          raceSignal,
          AbortSignal.timeout(PER_ENGINE_MS),
        ]);
        try {
          const result = await callEngine(engine, engineSignal);
          if (!result || result.sources.length === 0) throw new Error("returned 0 results");
          return { engine, result };
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          if (engine === preferred) preferredFailure = message;
          failures.push(`${engine}: ${message}`);
          logger.warn(`free-search: engine "${engine}" failed (${message})`);
          throw error;
        }
      };

      const won = await hedgedRace(leader, racers, lastResort, runEngine, { signal });
      if (won === null) {
        throw new Error(
          failures.length > 0
  ...
  const runEngineTest = async (engine, query, timeRange) => {
    ...
    try {
      ...
      if (result.ok === false) return result;
      if (!result.sources || result.sources.length === 0) {
        return { ok: false, error: `${engine} returned 0 results after retry` };
      }
      return {
        ok: true,
